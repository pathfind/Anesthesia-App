...
static NSString * const kStrykerAnesthesiaJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_strykeranesthesiaInjection
{
	...
	[html appendString:kStrykerAnesthesiaJavascript];
...
{
	// attempt to make well-formed HTML and inject in our StrykerAnesthesia bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _strykeranesthesiaInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to StrykerAnesthesia support.",url);
				}
			}
