...
@implementation StrykerAnesthesiaObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_strykeranesthesia);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any StrykerAnesthesia patches in JS before a context is loaded... nice for 
...
{
	// create StrykerAnesthesia global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_strykeranesthesia = [[StrykerAnesthesiaObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_strykeranesthesia];
	
	NSString *_strykeranesthesiaNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _strykeranesthesiaNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_strykeranesthesia valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.strykeranesthesia.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
